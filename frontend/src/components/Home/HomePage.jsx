...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const resBooks = await bookApi.getBooks({ per_page: 20, sort: "newest" });
        let booksArray = [];
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center text-primary font-bold">Đang tải dữ liệu...</div>;
  }
    ...
    <div className="flex flex-col gap-10 pb-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 relative z-10 gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="home-flash-sale-copy text-xs font-medium uppercase tracking-widest">Ưu đãi giới hạn mỗi ngày</p>
                </div>
              </div>
      ...
      <section className="container mx-auto px-8 lg:px-10">
        ...
        <div className="app-section-divider flex justify-between items-end mb-8 pb-5">
          <div className="flex flex-col gap-1">
            <h2 className="app-section-title text-2xl font-bold uppercase">Sách Mới Phát Hành</h2>
            <div className="app-section-kicker h-1.5 w-20 rounded-full"></div>
          </div>
          <Link to="/catalog?sort=newest" className="app-primary-link text-sm font-bold flex items-center gap-1">
      ...
      <section className="container mx-auto px-8 lg:px-10">
        ...
        <div className="text-center mb-10">
          <h2 className="app-section-title text-3xl font-bold uppercase mb-2">Gợi Ý Cho Bạn</h2>
          <p className="app-muted-text text-sm">Dựa trên sở thích đọc sách của bạn</p>
        </div>
        ...
        <div className="flex justify-center mt-12">
          <Link to="/catalog" className="app-outline-button px-10 py-3 border-2 font-bold rounded-xl transition-all duration-300 shadow-sm">
            XEM THÊM SẢN PHẨM
          </Link>
        </div>
        </div>
