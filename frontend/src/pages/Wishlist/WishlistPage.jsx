    ...
    <div className="min-h-screen pb-12">
      <div className="bg-white py-3 border-b border-gray-200 mb-8">
        <div className="container mx-auto px-8 lg:px-10 text-sm text-gray-600 flex items-center gap-2">
          ...
          <span className="text-primary font-medium">Danh sách yêu thích</span>
        </div>
      </div>
      ...
      <div className="container mx-auto px-8 lg:px-10">
        ...
        {wishlistItems.length === 0 ? (
          <div className="bg-white p-10 rounded-lg shadow-sm text-center border border-gray-100">
            ...
            <Link to="/" className="inline-block bg-primary text-white px-6 py-2 rounded-md hover:bg-green-800 transition shadow-sm">
              Khám phá sách ngay
            </Link>
          </div>
        ) : (
                ...
                <div key={item.id} className={`p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-gray-100 ${index !== wishlistItems.length - 1 ? 'border-b' : ''}`}>
                  ...
                  <div className="flex items-center gap-5 w-full md:w-1/2">
                    ...
                    <div className="flex flex-col">
                      <Link to={`/book/${item.slug || item.id}`} className="text-base font-medium text-gray-800 hover:text-primary line-clamp-2 mb-1">
                        {itemTitle}
                      </Link>
                      <div className="text-sm text-gray-500 mb-2">{itemAuthor}</div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-primary text-lg">{formatCurrency(itemPrice)}</span>
                  ...
                  <div className="flex items-center gap-4 ml-25 md:ml-0">
                    <button 
                      ...
                      className="flex-1 md:flex-none border-2 border-primary text-primary px-6 py-2.5 rounded font-bold hover:bg-green-50 transition flex justify-center items-center gap-2"
                    >
