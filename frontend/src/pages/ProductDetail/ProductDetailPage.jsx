...
const RatingStars = ({ rating }) => {
  ...
  return (
    <div className="flex">
      {[1, 2, 3, 4, 5].map((star) => {
        const fillPercent = Math.max(0, Math.min(1, normalizedRating - star + 1)) * 100;

        return (
          <span key={star} className="relative inline-flex w-4 h-4">
            <svg className="app-rating-star-base w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path d={STAR_PATH} />
            </svg>
            <span className="absolute inset-0 overflow-hidden" style={{ width: `${fillPercent}%` }}>
              <svg className="app-rating-star-fill w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path d={STAR_PATH} />
              </svg>
            </span>
          </span>
...
const ProductDetailPage = () => {
  ...
  const stock = Number.isFinite(Number(book?.available_stock))
    ? Number(book.available_stock)
    : 999;
  ...
  useEffect(() => {
    ...
    recommendationApi
      .trackBookView(book.id, "book_detail")
      .then(() => {
  ...
  const handleAddToCart = () => {
    ...
    if (quantity > stock)
      return toast.error(`Chỉ còn ${stock} sản phẩm trong kho!`);
  ...
  if (isLoading)
    return (
      <div className="py-20 flex justify-center">
        <div className="w-10 h-10 border-4 border-primary-strong border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  if (!book)
    return (
      <div className="py-20 text-center text-gray-500">Sách không tồn tại.</div>
    );
  ...
  if (book.flash_sale) {
    const fsDiscount = Number(
      book.flash_sale.discount_percent || book.flash_sale.discount_value || 0,
    );
    if (fsDiscount > 0) {
  ...
  const format =
    book.detail?.format_label || book.detail?.format || "Đang cập nhật";
  ...
  const description =
    book.detail?.description || "<p>Chưa có mô tả cho sản phẩm này.</p>";

  const averageRating = book.average_rating || 0;
  const reviewCount = book.review_count || 0;
  const galleryImages =
    book.images?.length > 0 ? book.images : [{ image_url: book.thumbnail_url }];
  ...
  return (
    <div className="min-h-screen pb-20 pt-4">
      <div className="container mx-auto px-8 lg:px-10 mb-4 text-sm text-gray-500 flex items-center gap-2">
        ...
        <Link to="/catalog" className="hover:text-primary-strong">
          Danh mục sách
        </Link>
        <FiChevronRight size={14} />
      ...
      <div className="container mx-auto px-8 lg:px-10">
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6 flex flex-col lg:flex-row gap-10">
                ...
                {galleryImages.map((img, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-16 h-16 border rounded cursor-pointer overflow-hidden ${mainImage === imageUrl ? "border-primary-strong border-2" : "border-gray-200"}`}
                    >
                      <img
                        src={imageUrl}
                        alt={`thumb-${idx}`}
          ...
          <div className="w-full lg:w-7/12">
            <h1 className="text-2xl font-semibold text-gray-800 mb-3 leading-snug">
              {title}
            </h1>
            ...
            <div className="flex items-end gap-4 mb-6">
              ...
              {originalPrice > sellingPrice && (
                <>
                  ...
                  <span className="bg-red-500 text-white text-sm font-semibold px-2 py-0.5 rounded mb-1.5">
                    -{discountPercent}%
                  </span>
                </>
              )}
            ...
            <div className="flex items-center gap-6 mb-8">
              <div className="flex items-center border border-gray-300 rounded h-10 w-28 bg-white">
                <button
                  ...
                  disabled={stock <= 0}
                  className="w-1/3 h-full flex justify-center items-center text-gray-600 hover:bg-gray-100 disabled:opacity-50"
                >
                  <FiMinus />
                </button>
                <input
                  ...
                  value={quantity}
                  readOnly
                  className="w-1/3 h-full text-center border-x border-gray-300 font-medium text-gray-800 outline-none"
                />
                <button
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          <div className="w-full lg:w-[60%] bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div
              className={`relative ${!isDescExpanded ? "max-h-64 overflow-hidden" : ""}`}
            >
              <div
          ...
          <div className="w-full lg:w-[40%] bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="flex flex-col gap-4 text-[14px]">
              <div className="flex">
                <span className="w-1/3 text-gray-500">Tác giả</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {authorDisplay}
                </span>
              </div>
              <div className="flex">
                <span className="w-1/3 text-gray-500">Dịch giả</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {translator}
                </span>
              </div>
              <div className="flex">
                <span className="w-1/3 text-gray-500">Nhà xuất bản</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {publisherName}
                </span>
              </div>
              ...
              <div className="flex">
                <span className="w-1/3 text-gray-500">Số trang</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {numPages}
                </span>
              </div>
              <div className="flex">
                <span className="w-1/3 text-gray-500">Khối lượng</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {weight !== "Đang cập nhật" ? `${weight} g` : weight}
                </span>
              </div>
              <div className="flex">
                <span className="w-1/3 text-gray-500">Ngày phát hành</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {pubYear}
                </span>
              </div>
              <div className="flex">
                <span className="w-1/3 text-gray-500">Hình thức</span>
                <span className="w-2/3 text-gray-800 font-semibold">
                  {format}
                </span>
              </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5 pb-4 border-b border-gray-100">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {reviewMeta?.total
                  ? `${reviewMeta.total} đánh giá đã được duyệt`
                  : "Các đánh giá từ khách hàng đã mua sách"}
              </p>
            </div>
                ...
                <div key={review.id} className="py-4 first:pt-0 last:pb-0">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    ...
                    <span className="text-amber-500 text-sm font-semibold">
                      {Number(review.rating || 0).toFixed(1)} sao
                    </span>
                    {review.created_at && (
