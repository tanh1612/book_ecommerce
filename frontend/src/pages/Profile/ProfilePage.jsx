...
const ProfilePage = () => {
  ...
  // 🔥 THUẬT TOÁN BÓC TÁCH DỮ LIỆU THÔNG MINH
  // Xử lý triệt để cấu trúc nested: { data: { email, profile: { first_name... } } }
  const extractUserData = (rawUser) => {
    if (!rawUser) return null;
    
    // Xuyên qua lớp vỏ 'data' của Axios/Laravel nếu có
    const coreData = rawUser.data || rawUser;
    
    // Xuyên qua lớp vỏ 'profile'
    const profileData = coreData.profile || {};

    return {
      email: coreData.email || profileData.email || "",
      first_name: profileData.first_name || coreData.first_name || "",
      last_name: profileData.last_name || coreData.last_name || "",
      phone: profileData.phone || coreData.phone || "",
      gender: (profileData.gender === 'other' || coreData.gender === 'other') 
              ? 'male' 
              : (profileData.gender || coreData.gender || "male"),
      birthday: profileData.birthday || coreData.birthday || ""
    };
  };

  const [formData, setFormData] = useState(() => {
    const initialData = extractUserData(user);
    return initialData || { email: "", first_name: "", last_name: "", phone: "", gender: "male", birthday: "" };
  });
  ...
  // Đồng bộ lại form nếu context user thay đổi
  useEffect(() => {
    if (user) {
            ...
            <div className="p-6 border-b border-gray-100 flex items-center gap-4 bg-[#f0f9f3]">
              ...
              <div>
                ...
                <div className="font-bold text-gray-800">
                  {formData.last_name || formData.first_name ? `${formData.last_name} ${formData.first_name}` : "Người dùng"}
                </div>
              </div>
            </div>
              ...
              <form onSubmit={handleUpdateProfile} className="max-w-2xl">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-8 mb-6">
                  ...
                  <div className="md:w-3/4">
                    <input 
                      type="text" 
                      value={formData.email || "Chưa cập nhật email"} 
                      disabled 
                      className="w-full border border-gray-200 bg-gray-100 rounded py-2 px-3 text-gray-800 font-medium cursor-not-allowed" 
                    />
                  </div>
                </div>
